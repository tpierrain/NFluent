        ...
        public static ICheckLink<ICheck<EventWaitHandle>> IsSetWithin(this ICheck<EventWaitHandle> check, double timeOut, TimeUnit timeUnit)
        {
            ...
            return checker.ExecuteCheck(
                () =>
                {
                    ...
                    {
                        var errorMessage = checker.BuildMessage(string.Format("The checked event has not been set before the given timeout.\nThe given timeout (in msec):\n\t[{0}]", timeOutInMsec)).ToString();
                        throw new FluentCheckException(errorMessage);
                    }
                },
                checker.BuildMessage(string.Format("The checked event has been set before the given timeout whereas it must not.\nThe given timeout (in msec):\n\t[{0}]", timeOutInMsec)).ToString());
        }
        ...
        public static ICheckLink<ICheck<EventWaitHandle>> IsNotSetWithin(this ICheck<EventWaitHandle> check, double timeOut, TimeUnit timeUnit)
        {
            ...
            return checker.ExecuteCheck(
                () =>
                {
                    if (checker.Value.WaitOne(timeOutInMsec))
                    {
                        var errorMessage = checker.BuildMessage(string.Format("The checked event has been set before the given timeout.\nThe given timeout (in msec):\n\t[{0}]", timeOutInMsec)).ToString();
                        throw new FluentCheckException(errorMessage);
                    }
                },
                checker.BuildMessage(string.Format("The checked event has not been set before the given timeout whereas it must.\nThe given timeout (in msec):\n\t[{0}]", timeOutInMsec)).ToString());
        }
    }
}
